...
DWORD WINAPI ThreadFunction(LPVOID lpParam)
    ...
    mute= 0;
    if (command[0] == '@') mute= 1;         // dont display command if @
...
void ReadFromPipe(BOOL convert)
    ...
    while(1)
        ...
        if((! bSuccess) || (dwRead == 0)) break;
            ...
            if (convert) CodePage(stBuf->buffer);           // if command = cmd.exe

            if (startBuf == NULL) startBuf= stBuf;
            if (bufptr != NULL) bufptr->next= stBuf;
...
//CONSOLE_FONT_INFOEX confnt;
    ...
    switch (message)
    {
        ...
        case WM_PAINT:
            ...
            if (scroll+maxaff > total) scroll= max(total-maxaff, 0);
        ...
        case WM_DESTROY:
            if (hPol != NULL) DeleteObject(hPol);
