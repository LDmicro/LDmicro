...
int PWM_Prediv(long fmcu, long fpwm)
	...
	if (q <= 256)		return(1);		// predivision = 1	
	if (q <= 1024)		return(4);		// predivision = 4	
	if (q <= 4096)		return(16);		// predivision = 16	
...
void PWM_Init(int canal, long fmcu, long fpwm, int resol)
	...
	if (cs == 0)	// fpwm trop petite
		{
		prescal= 0x02; pr2= 255;
		}
	if (cs == 1)
		{
		prescal= 0x01;
		if (q > 2) pr2= q-1; else pr2= 1;
		}
	if (cs == 4)
		{
		prescal= 0x01; pr2= (q/4)-1;
		}
	if (cs == 16)
		{
		prescal= 0x02; pr2= (q/16)-1;
		}
...
#if defined (LDTARGET_pic16f876) || defined (LDTARGET_pic16f877) || \
	defined (LDTARGET_pic16f886) || defined (LDTARGET_pic16f887)    
