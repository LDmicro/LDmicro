...
int PWM_Prediv(long fmcu, long fpwm, int csmax)
    ...
    if (csmax == 0) return (0);     // desactivera la pwm

    if (csmax == 5)         // pwm a 5 possibilites de predivision
        {
        if (p >= 1024)      return(5);      // cs (tres) approximatif
        if (p >= 256)       return(4);
        if (p >= 64)        return(3);
        if (p >= 8)         return(2);
        if (p >= 1)         return(1);
        ...
        {
        if (p >= 1024)      return(7);      // cs (tres) approximatif
        if (p >= 256)       return(6);
        if (p >= 128)       return(5);
        if (p >= 64)        return(4);
        if (p >= 32)        return(3);
...
void PWM_Stop(int canal)
    {
    if (canal == 0x1A) TCCR1A &= ~(1 << COM1A1);
    if (canal == 0x1B) TCCR1A &= ~(1 << COM1B1);
    if (canal == 0x02) TCCR2 &= ~(1 << COM21);
    }
#endif
...
#if defined (LDTARGET_atmega48) || defined (LDTARGET_atmega88) || \
    defined (LDTARGET_atmega168) || defined (LDTARGET_atmega328)
// set fast PWM mode with non-inverted output
void PWM_Init(int canal, long fmcu, long fpwm, int resol, int csmax)
    {
    int cs= PWM_Prediv(fmcu, fpwm, csmax);
...
void PWM_Set(int canal, unsigned percent, int resol)
    ...
    if (canal == 0x0A)  OCR0A= value;
    if (canal == 0x0B)  OCR0B= value;
    if (canal == 0x2A)  OCR2A= value;
    if (canal == 0x2B)  OCR2B= value;
    }

void PWM_Stop(int canal)
    {
    if (canal == 0x0A) TCCR0A &= ~(1 << COM0A1);
    if (canal == 0x0B) TCCR0A &= ~(1 << COM0B1);
    if (canal == 0x2A) TCCR2A &= ~(1 << COM2A1);
    if (canal == 0x2B) TCCR2A &= ~(1 << COM2B1);
    }
#endif


#if defined (LDTARGET_atmega164) || defined (LDTARGET_atmega324) || \
    defined (LDTARGET_atmega644) || defined (LDTARGET_atmega1284)
// set fast PWM mode with non-inverted output
void PWM_Init(int canal, long fmcu, long fpwm, int resol, int csmax)
    {
    int cs= PWM_Prediv(fmcu, fpwm, csmax);
...
void PWM_Set(int canal, unsigned percent, int resol)
    ...
    if (canal == 0x1A) OCR1A= value;
    if (canal == 0x1B) OCR1B= value;
    if (canal == 0x1C) OCR1C= value;
    if (canal == 0x2A) OCR2A= value;
    if (canal == 0x2B) OCR2B= value;
    if (canal == 0x3A) OCR3A= value;
    if (canal == 0x3B) OCR3B= value;
    if (canal == 0x3C) OCR3C= value;
    if (canal == 0x4A) OCR4A= value;
    if (canal == 0x4B) OCR4B= value;
    if (canal == 0x4C) OCR4C= value;
    }
...
void PWM_Stop(int canal)
    ...
    if (canal == 0x1A) TCCR1A &= ~(1 << COM1A1);
    if (canal == 0x1B) TCCR1A &= ~(1 << COM1B1);
    if (canal == 0x1C) TCCR1A &= ~(1 << COM1C1);
    if (canal == 0x2A) TCCR2A &= ~(1 << COM2A1);
    if (canal == 0x2B) TCCR2A &= ~(1 << COM2B1);
    if (canal == 0x3A) TCCR3A &= ~(1 << COM3A1);
    if (canal == 0x3B) TCCR3A &= ~(1 << COM3B1);
    if (canal == 0x3C) TCCR3A &= ~(1 << COM3C1);
    if (canal == 0x4A) TCCR4A &= ~(1 << COM4A1);
    if (canal == 0x4B) TCCR4A &= ~(1 << COM4B1);
    if (canal == 0x4C) TCCR4A &= ~(1 << COM4C1);
    }
#endif
