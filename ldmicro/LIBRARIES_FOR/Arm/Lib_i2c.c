...
void LibI2C_SetReg(I2C_TypeDef* I2Cx, uint8_t address, uint8_t reg, uint8_t data)
	...
	if ((address == LCD_I2C_ADR) && (reg == LCD_I2C_REG))		// traitement special afficheur
		...
		if (data <= 0x10)		// commandes diverses
			{
			if (data == 0) LCD_I2C_Erase();				// effacement ecran
			if (data == 1) LCD_I2C_Home();				// retour en haut a gauche
			if (data == 2) LCD_I2C_Config(1, 0, 1);		// clignotement active
			if (data == 3) LCD_I2C_Config(1, 0, 0);		// clignotement desactive
			}
