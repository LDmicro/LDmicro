...
static __INLINE uint32_t SysTick_Conf(uint32_t ticks)
{
  if (ticks > SysTick_LOAD_RELOAD_Msk)  return (1);            /* Reload value impossible */
...
static __INLINE uint32_t SysTick_COUNTFLAG()
{
  return SysTick->CTRL & SysTick_CTRL_COUNTFLAG_Msk;
}

static __INLINE void SysTick_Reset()
{
  SysTick->VAL = 0;                                            /* Load the SysTick Counter Value */
  return;
}

static __INLINE void SysTick_Off()
{
