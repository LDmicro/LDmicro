...
void LCD_I2C_Send4msb(char donnee)
{
    ...
    // Ecriture des 4 MSB
    if(donnee & (1 << 4)) {
        PORT_LCD(|= (1 << BIT_LCD_D4))
    } // Bit D4 à 1

    if(donnee & (1 << 5)) {
        PORT_LCD(|= (1 << BIT_LCD_D5))
    } // Bit D5 à 1

    if(donnee & (1 << 6)) {
        PORT_LCD(|= (1 << BIT_LCD_D6))
    } // Bit D6 à 1

    if(donnee & (1 << 7)) {
        PORT_LCD(|= (1 << BIT_LCD_D7))
    } // Bit D7 à 1
...
{
    LCD_I2C_SendCommand(LCD_CMD_DISPLAY | (affichage_actif << LCD_BIT_DISP_DISP) | (curseur_actif << LCD_BIT_DISP_CURS) | (clignotement_actif << LCD_BIT_DISP_BLINK));
}

// Déplacement affichage ou curseur
void LCD_I2C_Move(int type, int sens)
{
    LCD_I2C_SendCommand(LCD_CMD_SHIFT | (type << LCD_BIT_SHIFT_TYPE) | (sens << LCD_BIT_SHIFT_SENS));
}
