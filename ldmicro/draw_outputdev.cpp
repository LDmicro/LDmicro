...
//-----------------------------------------------------------------------------
// Âîçâđŕůŕĺěîĺ çíŕ÷ĺíčĺ - â ńëó÷ŕĺ óńďĺőŕ TRUE, číŕ÷ĺ FALSE
// hFile - äĺńęđčďňîđ ôŕéëŕ
// lpszString - óęŕçŕňĺëü íŕ áóôĺđ äë˙ ńňđîęč
...
BOOL GetLastWriteTime(HANDLE hFile, char *lpszString)
{
    ...
    // Ďîëó÷ŕĺě âđĺěĺíŕ ôŕéëŕ.
    if (!GetFileTime(hFile, &ftCreate, &ftAccess, &ftWrite))
        return FALSE;

    // Ďđĺîáđŕçóĺě âđĺě˙ ďîńëĺäíĺăî čçěĺíĺíč˙ â ëîęŕëüíîĺ âđĺě˙.
    FileTimeToSystemTime(&ftWrite, &stUTC);
    SystemTimeToTzSpecificLocalTime(NULL, &stUTC, &stLocal);

    // Ńîńňŕâë˙ĺě ńňđîęó ń äŕňîé č âđĺěĺíĺě.
...
BOOL sGetLastWriteTime(char *FileName, char *sFileTime)
{
    ...
    HANDLE hFile = CreateFile(FileName,   // îňęđűâŕĺěűé ôŕéë
                   GENERIC_READ,          // îňęđűâŕĺě äë˙ ÷ňĺíč˙
                   FILE_SHARE_READ,       // äë˙ ńîâěĺńňíîăî ÷ňĺíč˙
                   NULL,                  // çŕůčňŕ ďî óěîë÷ŕíčţ
                   OPEN_EXISTING,         // ňîëüęî ńóůĺńňâóţůčé ôŕéë
                   FILE_ATTRIBUTE_NORMAL, // îáű÷íűé ôŕéë
                   NULL);                 // ŕňđčáóňîâ řŕáëîíŕ íĺň
