    ...
    for(i = 0; i < IntCodeLen; i++) {
        ...
        switch(IntCode[i].op) {
            ...
            case INT_SPI_COMPLETE:
                fprintf(f, "SPI_COMPLETE '%s', done? into '%s'", l->d.spi.name, IntCode[i].name1);
                break;
            case INT_SPI_BUSY:
                fprintf(f, "SPI_BUSY '%s', done? into '%s'", l->d.spi.name, IntCode[i].name1);
                break;
            case INT_SPI:
                fprintf(f,
...
// Special function
        ...
        case ELEM_OSC: {
            ...
            */
            ...
            {
                case ELEM_7SEG:
                    Comment(3, stringer(ELEM_7SEG));
                    goto xseg;
                case ELEM_9SEG:
                    Comment(3, stringer(ELEM_9SEG));
                    goto xseg;
                case ELEM_14SEG:
                    Comment(3, stringer(ELEM_14SEG));
                    goto xseg;
                case ELEM_16SEG:
                    Comment(3, stringer(ELEM_16SEG));
                    goto xseg;
                xseg : {
                    break;
                }
...
/**/
        ...
        case ELEM_PIECEWISE_LINEAR: {
            ...
            for(i = 1; i < t->count; i++) {
                if(t->vals[i * 2] <= xThis) {
                    Error(
                        _("x values in piecewise linear table must be "
                          "strictly increasing."));
                    CompileError();
                }
                xThis = t->vals[i * 2];
        ...
        case ELEM_FORMATTED_STRING: {
            ...
            while(*p) {
                if(*p == '\\' && (isdigit(p[1]) || p[1] == '-')) {
                    if(digits >= 0) {
                        Error(
                            _("Multiple escapes (\\0-9) present in format "
                              "string, not allowed."));
                        CompileError();
                    }
                    p++;
                    ...
                    if(!isdigit(*p) || (*p - '0') > 5 || *p == '0') {
                        Error(
                            _("Bad escape sequence following \\; for a "
            ...
            if(digits >= 0 && (strlen(var) == 0)) {
                Error(
                    _("Variable is interpolated into formatted string, but "
                      "none is specified."));
                CompileError();
            } else if(digits < 0 && (strlen(var) > 0)) {
                Error(
                    _("No variable is interpolated into formatted string, "
    ...
    if((which == ELEM_COIL) || (which == ELEM_SET_PWM)) { // ELEM_COIL is a special case, see above
        return;
    }
